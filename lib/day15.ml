...
let print_warehouse grid (px, py) =
  List.iteri grid ~f:(fun y row ->
      List.iteri row ~f:(fun x c ->
          if x = px && y = py then Stdio.printf "%c" c else Stdio.printf "%c" c);
      Stdio.printf "\n")
...
let print_position (row, col) = Stdio.printf "Position: (%d, %d)\n" row col

let parse_input input =
  let warehouse, moves =
    List.split_while ~f:(fun s -> String.(s <> "")) input
  in
  ...
  let parsed_moves = List.concat_map moves ~f:String.to_list in
  ...
  let swap_positions warehouse (x1, y1) (x2, y2) =
    List.mapi warehouse ~f:(fun y row ->
        List.mapi row ~f:(fun x char ->
            ...
            else if x = x2 && y = y2 then
              List.nth_exn (List.nth_exn warehouse y1) x1
            else char))
  in
...
let solve_part1 (input : string list) =
  ...
  let warehouse, _ =
    List.fold moves ~init:(warehouse, position)
      ~f:(fun (current_warehouse, current_position) direction ->
        move current_warehouse current_position direction)
  in
  List.mapi warehouse ~f:(fun y row ->
      List.mapi row ~f:(fun x char ->
          if Char.(char = 'O') then x + (100 * y) else 0))
  |> List.concat |> List.fold ~init:0 ~f:( + ) |> Int.to_string
...
let move_part2 warehouse (x, y) direction =
  ...
  let move_box warehouse (x, y) (dx, dy) =
      ...
      | 0, _ ->
          ...
          let left =
            ...
            | '[' | ']' ->
                ...
                (blx, bly, blx, bly + dy)
                :: find_boxes warehouse (blx, bly) (dx, dy)
    ...
    let boxes =
      (nx, ny, nx + dx, ny + dy) :: find_boxes warehouse (nx, ny) (dx, dy)
    in
    ...
    if List.mem boxes (-1, -1, -1, -1) ~equal:equal_swap then (warehouse, (x, y))
    else
      let warehouse =
        List.mapi warehouse ~f:(fun y row ->
            List.mapi row ~f:(fun x char ->
      ...
      let warehouse =
        List.mapi warehouse ~f:(fun y row ->
            List.mapi row ~f:(fun x char ->
                if
                  List.mem boxes (0, 0, x, y)
                    ~equal:(fun (_, _, c1, d1) (_, _, c2, d2) ->
                      c1 = c2 && d1 = d2)
                then '['
                else if
                  List.mem boxes
