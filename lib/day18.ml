    ...
    | Some ((x, y), dist) ->
      if x = fst goal && y = snd goal then Some dist
        ...
        List.iter neighbors ~f:(fun (nx, ny) ->
          if nx >= 0 && nx < dimx && ny >= 0 && ny < dimy && memory.(ny).(nx) = 0 then
            Queue.enqueue queue ((nx, ny), dist + 1)
        );
        bfs ()
      )
  in
...
let solve_part1 (input: string list) =
  let memory = Array.make_matrix ~dimx:dimx ~dimy:dimy 0 in
...
let solve_part2 (input: string list) = 
  ...
  |> (fun (x, y) -> Int.to_string x ^ "," ^ Int.to_string y)