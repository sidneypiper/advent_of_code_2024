...
let parse_input (lines : string list) :
  ...
  let extract_button line =
    ...
    let x =
      String.chop_prefix_exn ~prefix:"X+" (List.nth_exn parts 2)
  ...
  let extract_prize line =
    let parts = String.split ~on:' ' line in
    let x =
      String.chop_prefix_exn ~prefix:"X=" (List.nth_exn parts 1)
      |> String.chop_suffix_exn ~suffix:","
    in
    let y = String.chop_prefix_exn ~prefix:"Y=" (List.last_exn parts) in
    (Float.of_string x, Float.of_string y)
  ...
  let rec parse_blocks acc = function
    | a :: b :: prize :: _ :: rest | a :: b :: prize :: rest ->
...
let solve_part1 (input : string list) =
  parse_input input |> List.map ~f:calc_presses |> List.fold ~init:0 ~f:( + )
  |> Int.to_string
