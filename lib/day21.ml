...
let parse_input input = List.map ~f:(String.append "A") input
...
let path_on_num_pad (a, b) =
  ...
  else if dx = 0 && dy = 0 then "A"
  else if dx > 0 && dy = 0 then String.concat [ repeat '>' (Int.abs dx); "A" ]
  else if dx < 0 && dy = 0 then String.concat [ repeat '<' (Int.abs dx); "A" ]
  else if dx = 0 && dy > 0 then String.concat [ repeat '^' (Int.abs dy); "A" ]
  else if dx = 0 && dy < 0 then String.concat [ repeat 'v' (Int.abs dy); "A" ]
  ...
  else String.concat [ repeat '<' (Int.abs dx); repeat 'v' (Int.abs dy); "A" ]
  ...
  let rec aux code n =
    Hashtbl.find_or_add memo (code, n) ~default:(fun () ->
        if n = 0 then String.length code
          ...
          List.fold ~init:0 ~f:( + )
            (List.map
               ~f:(fun (a, b) -> aux (path_on_dir_pad (a, b)) (n - 1))
               (get_pairs ("A" ^ code))))
  in
...
let solve_part1 input = solve input 2
let solve_part2 input = solve input 25
