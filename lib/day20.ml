...
let parse_input input = Array.of_list (List.map ~f:String.to_array input)
      ...
      let rec aux acc = function
        ...
        | ((x, y), dist) :: rest ->
            List.fold rest ~init:acc ~f:(fun acc ((nx, ny), d) ->
                if manhattan_distance (x, y) (nx, ny) = 2 && dist - d - 2 >= 100
                then acc + 1
                else acc)
            |> fun acc -> aux acc rest
      in
      aux 0 path |> Int.to_string
  | None -> "No path found"
