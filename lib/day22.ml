...
let parse_input lines = List.map lines ~f:Int.of_string
...
let calc_prices numbers = List.map numbers ~f:(fun x -> x % 10)
...
let solve_part2 input =
  ...
  List.iter parsed ~f:(fun seed ->
      ...
      let set =
        patterns_with_price patterns prices |> create_pattern_price_set
      in
