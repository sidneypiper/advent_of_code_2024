...
let parse_input input =
  ...
  | towels :: _ :: patterns ->
      let towels =
        String.substr_replace_all ~pattern:" " ~with_:"" towels
        |> String.split ~on:','
      in
      (towels, patterns)
  ...
  | None ->
      let result =
        if String.is_empty pattern then 1
          ...
          let chopped =
            List.filter_map towels ~f:(fun towel ->
                String.chop_prefix ~prefix:towel pattern)
          in
...
let solve_part1 (input : string list) =
  ...
  List.filter patterns ~f:(fun pattern ->
      count_possibilities towels pattern >= 1)
...
let solve_part2 (input : string list) =
  ...
  List.sum
    (module Int)
    patterns
    ~f:(fun pattern -> count_possibilities towels pattern)
  |> Int.to_string
