...
let parse_input_part2 input =
  ...
  let rec helper stones map =
    match stones with
    | [] -> map
    | x :: xs -> helper xs (IntMap.add x 1 map)
  in
...
let step_part2 stones =
  ...
  in
  IntMap.fold (fun key value acc -> apply_rules key value acc) stones IntMap.empty
...
let solve_part2 (input: string list) = 
  ...
  IntMap.fold (fun _ value acc -> acc + value) !stones 0 |>
  string_of_int