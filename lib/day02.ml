...
let is_save_diff (numbers : int list) =
  for_all (fun x y -> abs (x - y) > 0 && abs (x - y) < 4) numbers

let is_save_layer_part1 layer =
  is_save_diff layer && (is_asc layer || is_desc layer)

let solve f numbers =
  let safe_list = List.map f numbers in
  string_of_int
  @@ List.fold_left ( + ) 0 (List.map (fun x -> if x then 1 else 0) safe_list)
