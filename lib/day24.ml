...
module Gate = struct
  ...
  let apply (cables : Wires.t) (gate : t) : Wires.t =
    ...
    | AND (input1, input2, output) -> (
        let i1 = Map.find cables input1 and i2 = Map.find cables input2 in
        match (i1, i2) with
...
let parse_input (input : string list) : Wires.t * Gate.t list =
  ...
  let gates =
    List.drop_while input ~f:(fun x -> not @@ String.equal "" x) |> List.tl_exn
  in
...
let simulate_circuit (wires : Wires.t) (gates : Gate.t list) : Wires.t =
  let rec aux current_wires =
    ...
    if Map.equal Bool.equal current_wires new_wires then new_wires
    else aux new_wires
  in
  aux wires
...
let get_output_gates (gates : Gate.t list) (output : string) : Gate.t list =
  List.filter gates ~f:(fun gate ->
      match gate with
      | Gate.AND (input1, input2, _)
      | Gate.OR (input1, input2, _)
      | Gate.XOR (input1, input2, _) ->
          String.equal input1 output || String.equal input2 output)

let is_gate_valid (gate : Gate.t) (gates : Gate.t list) : bool =
  let output_gates output = get_output_gates gates output in
  match gate with
  ...
  | Gate.XOR (input1, input2, output) -> (
      ...
      | [ Gate.AND _; Gate.XOR _ ] | [ Gate.XOR _; Gate.AND _ ] ->
          String.is_prefix ~prefix:"x" input1
          && String.is_prefix ~prefix:"y" input2
          || String.is_prefix ~prefix:"x" input2
             && String.is_prefix ~prefix:"y" input1
      | _ -> false)

let solve_part2 input =
  let _, gates = parse_input input in
  let valid_gates =
    List.filter gates ~f:(fun gate -> not (is_gate_valid gate gates))
  in
