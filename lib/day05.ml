...
let split_on_empty_line lines =
  let rec helper lines acc1 acc2 in_second_list =
    ...
    | line :: rest ->
        if in_second_list then
          helper rest acc1 (line :: acc2) true
        else
          helper rest (line :: acc1) acc2 false
  in
  helper lines [] [] false  
...
let compare graph a b =
  if List.mem b (IntMap.find a graph) then
    -1
  else if List.mem a (IntMap.find b graph) then
    1
  else
    0

let sort_list rules lst =
  let compare_values x y = compare rules x y
  in
...
let solve_part1 (input: string list) = 
  ...
  let valid_list = List.map (fun x -> 
    if x = sort_list rules x then middle_element x else 0) update in
...
let solve_part2 (input: string list) = 
  ...
  let valid_list = List.map (fun x -> 
    ...
    if x = sorted then 0 else middle_element sorted) update in
  string_of_int @@ List.fold_left (+) 0 valid_list