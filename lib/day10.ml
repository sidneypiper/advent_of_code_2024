let parse_to_2d_list (input : string list) : int list list =
  let parse_row (row : string) : int list =
    List.init (String.length row) (fun i -> int_of_char row.[i] - int_of_char '0')
  in
    ...
    try
      if Hashtbl.mem visited (x, y) then reachable_count
        ...
        if current_value <> target_value then reachable_count
          ...
          let new_count = if target_value = 9 then reachable_count + 1 else reachable_count in
          let neighbors = [(x - 1, y); (x + 1, y); (x, y - 1); (x, y + 1)] in
          List.fold_left (fun acc (nx, ny) -> dfs nx ny (target_value + 1) acc) new_count neighbors
        )
  ...
  for x = 0 to List.length graph - 1 do
    for y = 0 to List.length graph - 1 do
      if List.nth (List.nth graph x) y = 0 then
        n := !n + reachable_9 graph x y
    done
  done;
  string_of_int !n

let count_paths_to_9 (graph : int list list) (start_x : int) (start_y : int) : int =
  let rec dfs x y target_value =
    try
      ...
      if current_value <> target_value then 0
        ...
        let neighbors = [(x - 1, y); (x + 1, y); (x, y - 1); (x, y + 1)] in
        List.fold_left (fun acc (nx, ny) -> acc + dfs nx ny (target_value + 1)) 0 neighbors
    with _ -> 0
  in
