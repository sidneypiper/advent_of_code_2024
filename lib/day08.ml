...
let parse_input (input : string list) =
  ...
  for y = 0 to List.length input - 1 do
    ...
    for x = 0 to String.length line - 1 do
      ...
      if c <> '.' then map := add_to_map !map c (x, y)
    done
  done;
  !map
...
let generate_all_pairs map =
  CharMap.fold (fun _ positions acc -> acc @ generate_pairs positions) map []
...
let remove_duplicates (vectors : vector list) =
  let set =
    List.fold_left (fun acc v -> VectorSet.add v acc) VectorSet.empty vectors
  in
...
let solve_part1 (input : string list) =
  ...
  in
  List.length antinodes |> string_of_int
