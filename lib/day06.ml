...
let set_char_at map x y new_char =
  ...
  let updated_map = List.mapi (fun i r -> if i = y then updated_row else r) map in
  updated_map
...
let step map x y = 
  ...
  let rec helper map x y d visited =
    if x > 0 && x < max_x - 1 && y > 0 && y < max_y - 1 then 
      ...
      | 1 ->  if get_char_at map (x + 1) y = '#' then helper map x y 2 visited
              else helper map (x + 1) y d visited
      | 2 ->  if get_char_at map x (y + 1) = '#' then helper map x y 3 visited
              else helper map x (y + 1) d visited
