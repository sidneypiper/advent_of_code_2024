...
let parse_input (input : string list) : (int * int * int * int) list =
  let extract_numbers line =
    ...
    let p =
      String.chop_prefix_exn ~prefix:"p=" (List.nth_exn parts 0)
      |> String.split ~on:','
    in
    let v =
      String.chop_prefix_exn ~prefix:"v=" (List.nth_exn parts 1)
      |> String.split ~on:','
    in
    let px, py, vx, vy =
      (List.nth_exn p 0, List.nth_exn p 1, List.nth_exn v 0, List.nth_exn v 1)
    in
...
let solve_part1 (input : string list) =
  let quadrants =
    parse_input input |> List.map ~f:(fun x -> calc_quadrant x 100)
  in
...
let find_tree (robots : (int * int * int * int) list) : int =
  let rec helper robots seconds =
    ...
    if a > b + c + d || b > a + c + d || c > a + b + d || d > a + b + c then
      seconds
    else helper robots (seconds + 1)
  in
  helper robots 0

let solve_part2 (input : string list) =
  parse_input input |> find_tree |> Int.to_string
