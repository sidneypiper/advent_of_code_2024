...
let extract_do_mul_occurrences input =
  ...
  let rec collect_matches acc pos =
    try
      ...
      let do_pos = try search_backward (regexp {|\(do()\)|}) input mul_pos with Not_found -> 0 in
      let dont_pos = try search_backward (regexp {|\(don\'t()\)|}) input mul_pos with Not_found -> max_int in
      if do_pos > dont_pos || dont_pos = max_int then 
        collect_matches ((int_of_string x, int_of_string y) :: acc) ende
      else
        collect_matches acc ende
    with
      Not_found -> acc
  in
  collect_matches [] 0
